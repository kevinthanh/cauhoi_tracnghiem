...
class DapanController extends BaseController {
	...
	public function postEdit() {
		$dapan_model = new Dapan();
		$id = Input::get('dapan_id');
		$dapan_model = Dapan::find($id);
		$dapan_model->cauhoi_id		= Input::get('cauhoi');
		$dapan_model->tendapan		= Input::get('tendapan');
		$dapan_model->dapan_dung	= Input::get('dapan_dung');
		$dapan_model->save();
		return Redirect::route('listdapan_get')->with('success','Sửa đáp án thành công');
	}
